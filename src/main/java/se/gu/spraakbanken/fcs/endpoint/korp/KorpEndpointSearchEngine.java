...
public class KorpEndpointSearchEngine extends SimpleEndpointSearchEngineBase {
    ...
    protected EndpointDescription createEndpointDescription(ServletContext context,
        ...
        try {
            ...
            throw new SRUConfigException("Malformed URL for initializing resource info inventory",
                    mue);
        }
    }

    /**
     * Initialize the search engine. This initialization should be tailored towards your environment
     * and needs.
     *
     * @param context            the {@link ServletContext} for the Servlet
     * @param config             the {@link SRUServerConfig} object for this search engine
     * @param queryParserBuilder the {@link SRUQueryParserRegistry.Builder} object to be used for
     *                           this search engine. Use to register additional query parsers with
     *                           the {@link SRUServer}.
     * @param params             additional parameters gathered from the Servlet configuration and
     *                           Servlet context.
     * @throws SRUConfigException if an error occurred
     */
    protected void doInit(ServletContext context, SRUServerConfig config,
            SRUQueryParserRegistry.Builder queryParserBuilder, Map<String, String> params)
    ...
    /**
     * Destroy the search engine. Override this method for any cleanup the search engine needs to
     * perform upon termination.
     */
    ...
    /**
     * Handle a <em>scan</em> operation. The default implementation is a no-op. Override this
     * method, if you want to provide a custom behavior.
     *
     * @param config      the <code>SRUEndpointConfig</code> object that contains the endpoint
     *                    configuration
     * @param request     the <code>SRURequest</code> object that contains the request made to the
     *                    endpoint
     * @param diagnostics the <code>SRUDiagnosticList</code> object for storing non-fatal
     *                    diagnostics
     * @return a <code>SRUScanResultSet</code> object or <code>null</code> if this operation is not
     *         supported by this search engine
     * @throws SRUException if an fatal error occurred
     */
    @Override
    protected SRUScanResultSet doScan(SRUServerConfig config, SRURequest request,
    ...
    /**
     * Convenience method for parsing a string to boolean. Values <code>1</code>, <code>true</code>,
     * <code>yes</code> yield a <em>true</em> boolean value as a result, all others (including
     * <code>null</code>) a <em>false</em> boolean value.
     *
     * @param value the string to parse
     * @return <code>true</code> if the supplied string was considered something representing a
     *         <em>true</em> boolean value, <code>false</code> otherwise
     */
    ...
    private void writeEndpointDescription(XMLStreamWriter writer) throws XMLStreamException {
        ...
        for (DataView dataView : endpointDescription.getSupportedDataViews()) {
            ...
            switch (dataView.getDeliveryPolicy()) {
                ...
                case NEED_TO_REQUEST:
                    ...
                    break;
                default:
                    throw new XMLStreamException("invalid value for payload delivery policy: "
        ...
        try {
            List<ResourceInfo> resources =
                    endpointDescription.getResourceList(EndpointDescription.PID_ROOT);
    ...
    private void writeResourceInfos(XMLStreamWriter writer, List<ResourceInfo> resources)
    ...
    public SRUSearchResultSet search(SRUServerConfig config, SRURequest request,
            SRUDiagnosticList diagnostics) throws SRUException {
        String query;
        if (request.isQueryType(Constants.FCS_QUERY_TYPE_CQL)) {
            ...
            throw new SRUException(SRUConstants.SRU_CANNOT_PROCESS_QUERY_REASON_UNKNOWN,
                    "Queries with queryType '" + request.getQueryType()
        ...
        if (hasFcsContextCorpus && !"".equals(fcsContextCorpus)) {
            if (!"hdl%3A20.500.12115%2F9".equals(fcsContextCorpus)) {
    ...
    protected Query makeQuery(final String cqpQuery, CorporaInfo openCorporaInfo,
            final int startRecord, final int maximumRecords) {
