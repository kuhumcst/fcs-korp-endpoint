...
public class FCSToCQPConverter {
	 ...
	 * @throws eu.clarin.sru.server.SRUException If the query is too complex or it for any other
	 *         reason cannot be performed
	 */
	public static String makeCQPFromCQL(final SRUQuery<CQLNode> query) throws SRUException {
		final CQLNode node = query.getParsedQuery();
		/*
		 * Translate the CQL query to a Lucene query. If a CQL feature was used, that is not
		 * supported by us, throw a SRU error (with a detailed error message)
		 *
		...
		if (node instanceof CQLBooleanNode) {
			...
			throw new SRUException(SRUConstants.SRU_UNSUPPORTED_BOOLEAN_OPERATOR, operator,
			...
			if (terms.length > 1) {
				...
				for (int i = 0; i < terms.length; i++) {
					if (terms[i].startsWith("\"") || terms[i].startsWith("'")
							|| terms[i].endsWith("\"") || terms[i].endsWith("'")) {
						String tmp = terms[i];
						if (terms[i].startsWith("\"") || terms[i].startsWith("'")) {
							tmp = tmp.substring(1);
						}
	...
	private static String getExpressionBoolOp(final QueryNode op, final String opString)
			throws SRUException {
		List<String> children = new ArrayList<String>();
	...
	private static String getExpression(final Expression child) throws SRUException {
		...
		if ((expression.getLayerIdentifier().equals("text")
				|| expression.getLayerIdentifier().equals("token")
				|| expression.getLayerIdentifier().equals("word")
				|| expression.getLayerIdentifier().equals("lemma")
				|| expression.getLayerIdentifier().equals("pos"))
				&& (expression.getLayerQualifier() == null)
				&& (expression.getOperator() == Operator.EQUALS
						|| expression.getOperator() == Operator.NOT_EQUALS) // &&
		// (expression.getRegexFlags() == null)
			...
			if (expression.getLayerIdentifier().equals("pos")) {
				return translatePos(expression.getLayerIdentifier(),
						getOperator(expression.getOperator()), expression.getRegexValue());
	...
	private static String translatePos(final String layerIdentifier, final String operator,
			final String pos) throws SRUException {
	...
	private static String getWordLayerFilter(Expression expression) {
		...
		StringBuffer buf = new StringBuffer();
		buf.append((expression.getLayerIdentifier().equals("text")
				|| expression.getLayerIdentifier().equals("token")) ? "word"
