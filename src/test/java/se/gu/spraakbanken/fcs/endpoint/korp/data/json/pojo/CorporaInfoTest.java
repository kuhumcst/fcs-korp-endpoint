...
import se.gu.spraakbanken.fcs.endpoint.korp.data.json.pojo.info.CorpusAttrs;
import se.gu.spraakbanken.fcs.endpoint.korp.data.json.pojo.info.CorpusMetaInfo;
...
public class CorporaInfoTest {
	...
	public void corporaInfoDeserialize() {
		...
		try {
			ci2 = mapper.readerFor(CorporaInfo.class).readValue(jsonString);
	...
	public void corporaInfoDeserializeURL() {
		...
		String queryString = "indent=4&command=info&corpus=LSPCONSTRUCTIONEB1,LSPCONSTRUCTIONEB2";

		try {
			URL korp = new URL(wsString + queryString);

			ci4 = mapper.readerFor(CorporaInfo.class).readValue(korp.openStream());
