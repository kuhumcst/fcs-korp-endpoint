...
import se.gu.spraakbanken.fcs.endpoint.korp.data.json.pojo.statistics.Sums;
...
public class CountCQPTest {
    ...
    public void countCQPDeserialize() {
        ...
        try {
            cl2 = mapper.readerFor(CountCQP.class).readValue(jsonString);
    ...
    public void countCQPDeserializeURL() {
        ...
        String queryString =
                "indent=4&command=count&cqp=%5Bword=%22givende%22%5D&corpus=LSPCONSTRUCTIONEB1&groupby=word&ignore_case=word";
        // {"corpora":{},"time":3.287792205810547E-4,"ERROR":{"type":"KeyError","value":"'Key is
        // required: groupby'"}}

        try {
            URL korp = new URL(wsString + queryString);

            cl4 = mapper.readerFor(CountCQP.class).readValue(korp.openStream());
